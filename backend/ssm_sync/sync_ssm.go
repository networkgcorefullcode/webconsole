...
// List of actions: "SYNC_EXTERNAL_KEYS", "SYNC_USERS", "SYNC_OUR_KEYS", "HEALTH_CHECK"
...
func SyncSsm(ssmSyncMsg chan *SsmSyncMessage) {
	...
	for {
		...
		case <-time.After(time.Duration(factory.WebUIConfig.Configuration.SSM.SsmSync.IntervalMinute) * time.Minute):
...
func SyncKeys(keyLabel, action string) {
	...
	k4listChanSSM := make(chan []ssm.DataKeyInfo)
	...
	ssmKeysMap := make(map[string]ssm.DataKeyInfo)
		...
		go func() {
			...
			if subsData.AuthenticationSubscription.PermanentKey.EncryptionAlgorithm == 0 &&
				...
				var encryptRequest ssm.EncryptRequest = ssm.EncryptRequest{
					KeyLabel:            ssm_constants.LABEL_ENCRIPTION_KEY_AES256,
					Plain:               subsData.AuthenticationSubscription.PermanentKey.PermanentKeyValue,
					EncryptionAlgorithm: ssm_constants.ALGORITM_AES256_OurUsers,
				}
				...
				if resp.Cipher != "" {
					...
					newSubAuthData.PermanentKey.EncryptionAlgorithm = ssm_constants.ALGORITM_AES256_OurUsers
